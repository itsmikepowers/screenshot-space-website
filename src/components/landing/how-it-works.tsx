      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-4">
          {shortcuts.map((item, i) => (
            <div key={i} className="flex flex-col sm:flex-row sm:items-center gap-4 p-5 bg-white rounded-2xl border border-gray-200/60 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-center gap-3">
                <kbd className="px-3 py-2 sm:px-4 sm:py-2.5 bg-gradient-to-b from-gray-50 to-gray-100 border border-gray-300 rounded-xl text-sm sm:text-base font-medium shadow-sm whitespace-nowrap">
                  {item.key}
                </kbd>
                <span className="text-xs sm:text-sm font-medium text-gray-400 uppercase tracking-wide">{item.action}</span>
              </div>
              <div className="flex-1">
                <div className="font-semibold text-gray-900">{item.title}</div>
                <div className="text-sm sm:text-base text-gray-500">{item.desc}</div>
              </div>
